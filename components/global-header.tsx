...
import { usePlanningPeriod, availableMonths, type OptimizedScheduleRow } from "@/components/planning-period-context"
...
export function GlobalHeader({
  ...
  const { selectedMonth, setSelectedMonth, setOptimizedScheduleRows } = usePlanningPeriod()
  ...
  const handleGenerateOptimization = async () => {
    ...
    try {
      // Stub: simulate API call latency — replace with real fetch() when API is ready
      // e.g. const res = await fetch("/api/optimize", { method: "POST", body: JSON.stringify(payload) })
      // const data = await res.json()
      // setOptimizedScheduleRows(data.rows)
      await new Promise(resolve => setTimeout(resolve, 1500))

      // Placeholder empty result — will be populated by CSV upload on the scheduling page
      setOptimizedScheduleRows([])

      // Navigate to scheduling page where results are displayed
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  {isGenerating ? "Generating..." : "Generate Optimization"}
                </Button>
              </TooltipTrigger>
              <TooltipContent side="bottom">
                <p className="text-xs">Run the schedule optimizer and navigate to the Scheduling page</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
